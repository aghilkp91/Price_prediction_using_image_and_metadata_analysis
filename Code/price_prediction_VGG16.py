...
num_epochs = 20
checkpoint_path = data_path + '/output/VGG16_ecommerce-cnn-best.hdf5'
...
def run_model(train_indices, validation_indices, test_indices):
    ...
    for setting in range(num_settings):
        ...
        # plt.show()
        plt.savefig(os.path.abspath(os.path.dirname(os.path.realpath(__file__)) + '/..') + '/Data/output/result_VGG16_' + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + '.png')
        print("==========")
...
def predict_model_on_test():
        ...
        # with open(data_path + '/output/VGG16_result_scores.json', 'wb', encoding="utf-8") as file:
        #     file.write(results)
        with open(data_path + '/output/VGG16_result_scores_' + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + '.json', 'w') as file:
